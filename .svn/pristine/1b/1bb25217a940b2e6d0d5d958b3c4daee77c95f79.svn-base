...
{
    public partial class SendEmailToDonor : Page
    {
        ...
        protected void btnIncludeBalance_Click(object sender, EventArgs e)
        {
            if (includeBalance.Checked)
                txtMessage.Text = txtMessage.Text + "\r\n\r\nYour current balance is " + txtBalanceHidden.Value + "\r\n\r\n";
            else
            {
        ...
        protected void btnUpdate_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (donorId != 0)
            {
                ...
                if (EmailSender.SendEmailEx(Utils.fixNullString(txtEmailFrom.Text), donorId, new EmailEx
                {
                    ...
                    if (bReturn)
                        ...
                        Response.Redirect("ListDonors.aspx?status=5101001"); //  email sent successfully.
                else
                    if (bReturn)
                        Response.Redirect("GetDonationReport.aspx?status=5101000"); //  email not sent.
                    else
                        Response.Redirect("ListDonors.aspx?status=5101000"); //  email not sent.
            }
            else
            {
                if (EmailSender.SendEmailEx(Utils.fixNullString(txtEmailFrom.Text), donorId, new EmailEx
