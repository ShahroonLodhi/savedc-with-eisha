...
namespace SaveDC.ControlPanel
{
    public partial class GetSchoolExpenseReportDetail : Page
    {
        ...
        {
            if (Session["MonthlyExpenseReportDetail"] != null)
            {
                DataSet ds = Session["MonthlyExpenseReportDetail"] as DataSet;
                if (ds != null && ds.Tables.Count > 0)
                {
                    if (ds.Tables[0] != null && ds.Tables[0].Rows.Count > 0)
                    {
                        ExportToExcel(ds.Tables[0], "Monthly Expense Report for " + lblMonth.Text);
                    }
                }
            }
        }
        ...
        private void ExportToExcel(DataTable dt, string fileName)
        {
            ...
            try
            {
                if (dt.Rows.Count >= 1)
                {
                    ...
                    dt2.Columns["SchoolName"].ColumnName = "Payee Name";
                    dt2.Columns["ExpenseType"].ColumnName = "Expense Type";
                    dt2.Columns["PaymentMode"].ColumnName = "Payment Mode";
                    dt2.Columns["TotalExpenses"].ColumnName = "Expense Amount";
                    dt2.Columns["BankName"].ColumnName = "Bank";
                    dt2.Columns["PaymentDate"].ColumnName = "Posted On";

                    dt2.Columns["Payee Name"].SetOrdinal(0);
                    dt2.Columns["Expense Type"].SetOrdinal(1);
                    dt2.Columns["Expense Amount"].SetOrdinal(2);
                    dt2.Columns["Payment Mode"].SetOrdinal(3);
                    dt2.Columns["Bank"].SetOrdinal(4);
                    dt2.Columns["Posted On"].SetOrdinal(5);
        ...
        private void GetReport(int nCurrentPage)
        {
            ...
            DataSet report = oCommon.GetSchoolMonthlyExpenseReportDetail(szSchoolName, szExpMonth, nCurrentPage,
                                                                         ...
                                                                         out nTotalRecord);
            Session["MonthlyExpenseReportDetail"] = report;
            //===============================================================
