    ...
    public partial class Menu : UserControl
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (SaveDCSession.UserAccessLevel == UserAccessLevels.SuperAdmin ||
                ...
                liMembersArea.Visible = true;
            else
                liMembersArea.Visible = false;
            if (SaveDCSession.UserAccessLevel != UserAccessLevels.SuperAdmin)
            {
            ...
            if (SaveDCSession.UserAccessLevel == UserAccessLevels.Operator)
            {
                ...
                //{
                //    liExpenseManager.Visible = false;
                //    liFundsManager.Visible = false;
                //}

                //if (SaveDCSession.UserName.ToLower() == "muzaffar")
                //    liExpenseManager.Visible = false;

                //1/12/2022 work done for Operator level.
                liEmployeeManager.Visible = false;
                liMembersArea.Visible = false;
            }

            //3/20/2022 work done to hide the report link for username: Azhar
            if (SaveDCSession.UserAccessLevel == UserAccessLevels.Admin
                && SaveDCSession.UserName.ToLower() == SaveDCConstants.usernameazhar
                && SaveDCSession.UserId.Equals(SaveDCConstants.useridazhar))
            {
                liReports.Visible = false;
                liReports1.Visible = false;
            }
            ...
            else if (currentURL.Contains("employee"))
            {
                employee.Attributes.Add("style", "font-weight:bold");
            }
        }
    }
}