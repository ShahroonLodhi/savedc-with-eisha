    ...
    public partial class AddProgressReport : Page
    {
        ...
        protected void btnUpdate_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (nStatus > 0)
            {
                ...
                int donorId = Utils.fixNullInt(oStudent.DonorId);
                ...
                if (sendEmail.Checked == true)
                {
                    ...
                    string _mailbody = File.ReadAllText(EmailConfiguration.ReadConfigKey("PRMailBodyPath"));
                    _mailbody = string.Format(_mailbody, oUser.FirstName + " " + oUser.LastName);

                    EmailSender.SendEmailEx(EmailConfiguration.FromAddressInfo, donorId, new EmailEx
                    {
                        DonorName = Utils.fixNullString(oUser.UserName),
                        DonorEmail = Utils.fixNullString(oUser.EmailAddress),
                        Subject = "Progress Report Added",
                        Body = _mailbody
                    }, false);
                }
