...
namespace SaveDC.ControlPanel
{
    public partial class AddFund : Page
    {
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (SaveDCSession.UserAccessLevel != UserAccessLevels.Operator)
                {
                    ...
                    if (nEditFundId > 0)
                    {
                        ...
                        SetAttachInformation(oFund.Attachment);
                ...
                //Disabling the Email,Phone & balance in case of operator
                if(SaveDCSession.UserAccessLevel == UserAccessLevels.Operator)
                {
            ...
            try
            {
                ...
                if (hpf.ContentLength > 0)
                {
                    if (IsValidImageFile(Path.GetFileName(hpf.FileName)))
                    {
                        ...
                        hpf.SaveAs(Server.MapPath(SaveDCConstants.CertificateUploadPath) + szFileGUID + ".jpg");
                        return szFileGUID;
                    }
                }
            }
        ...
        private void SetAttachInformation(string szCertGuid)
        {
            string szServerPath = Server.MapPath(SaveDCConstants.CertificateUploadPath) + szCertGuid + ".jpg";
        ...
        protected void btnUpdate_Click(object sender, ImageClickEventArgs e)
        {
            ...
            oFund.Attachment = hdnNewCerti2.Value;
            ...
            if (nStatus > 0)
            {
                ...
                if (sendEmail.Checked == true)
                {
                    ...
                    if (oUser.UserName.ToLower().ToString() != "misc")
                    {
                        ...
                        EmailSender.SendEmailEx(EmailConfiguration.FromAddressAccounts, donorId, new EmailEx
                        {
                    ...
                    {
                        string _mailbody = File.ReadAllText(EmailConfiguration.ReadConfigKey("DonationMailBodyPath"));
                        _mailbody = string.Format(_mailbody, "Donor");

                        EmailSender.SendEmailEx(EmailConfiguration.FromAddressAccounts, donorId, new EmailEx
                        {
                            DonorName = Utils.fixNullString(donor_email_address.Text),
                            DonorEmail = Utils.fixNullString(donor_email_address.Text),
                            Subject = "Donation Received",
                            Body = _mailbody
                        }, false);
                    }
                ...
                if (sendSMS.Checked == true)
                {
                    ...
                    int statusCode = commom.SendSMSAndLogInDatabase(donorId, (donor_cell_no.ReadOnly) ? hdnUName.Value + "," + hdnPhoneNum.Value : hdnUName.Value + "," + donor_cell_no.Text,
                                                                    txtSMS.Text, "DON");
                ...
                if (nEditFundId > 0)
                    ...
                    Response.Redirect("ListFunds.aspx?status=5071001"); //Added successfully.
            }
            else
            {
        ...
        private void FillDonorDetails()
        {
            try
            {
                ...
                if (nDonorId > 0)
                {
                    ...
                    hdnUName.Value = (oUser.UserName.ToLower().ToString() != "misc") ? oUser.UserName : "donor";
                    donor_email_address.ReadOnly = (oUser.UserName.ToLower().ToString() == "misc") ? false : true;
                    donor_cell_no.ReadOnly = (oUser.UserName.ToLower().ToString() == "misc") ? false : true;
                    ...
                    if (SaveDCSession.UserAccessLevel == UserAccessLevels.SuperAdmin)
                    {
                        ...
                        donor_email_address.Text = oUser.EmailAddress;
                        donor_cell_no.Text = oUser.PhoneNumber;
                    }
