...
namespace SaveDC.ControlPanel
{
    public partial class AddAttachment : Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (nReportId > 0)
                {
                    ...
                    oCommon.GetStudentAttachmentDetail(nReportId, ref szNote, ref ImageGUID,
                                                          ref szAddedBy);
        ...
        protected void btnUpdate_Click(object sender, ImageClickEventArgs e)
        {
            ...
            int nStatus =
                nEditReportId =
                oCommon.AddStudentAttachment(nEditReportId, nStudentId,
                                                       szNote, szReportGUID,
            ...
            if (nStatus > 0)
            {
                ...
                try
                {
                    ...
                    {
                        string szServerPath = Server.MapPath(SaveDCConstants.CertificateUploadPath) + hdnOldReport.Value +
                                              ".jpg";
                        if (File.Exists(szServerPath))
                        {
            ...
            {
                if (bIsEdit)
                    Response.Redirect("ListAttachments.aspx?status=5031281"); //  
                else
                    Response.Redirect("ListAttachments.aspx?status=5031271");
            }
            else
            {
                Response.Redirect("AddAttachment.aspx?status=5031270");
            }
        }
